//! `docs/kangarootwelve.zig` is our optimized Zig “northern star”.
//! It’s intentionally written as one of the most beautiful examples of optimized Zig we’ve ever seen,
//! and we should aspire to bring our ops/kernels/etc up to the same idioms over time.
//!
//! Patterns to copy from here:
//! - SIMD-first `@Vector` code with clear scalar fallbacks
//! - cache-line alignment + false-sharing awareness
//! - explicit scratch buffers (no hidden allocations in hot paths)
//! - zero-copy views over discontiguous inputs (`MultiSliceView`)
//! - batching and thresholds chosen to avoid overhead on small inputs
//! - parallel work decomposition with careful ordering/collection
//!
