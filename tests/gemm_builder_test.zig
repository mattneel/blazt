...
fn computeRefGemm(
    ...
    for (0..m) |i| {
        for (0..n) |j| {
            ...
            c_out.atPtr(i, j).* = @floatCast(alpha_f * sum + beta_f * cin);
        }
    }
}
...
test "gemmBuilder configures ops.gemm and matches reference (layout/trans/alpha/beta)" {
    const T = f32;
...
test "gemmBuilder.parallel routes row_major to parallel.gemm and matches ops.gemm" {
    ...
    var rng = fx.FixtureRng.init(0x1234_5678_9abc_def0);
    const r = rng.random();
    ...
    // Keep initial C identical across both calls.
    ...
    try expectMatApproxEq(T, .row_major, c_ref, c_par, tol);
}
