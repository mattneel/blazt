...
test "gemm.computeTileParams returns sane comptime values (f32/f64)" {
    comptime {
        ...
        // conservative upper bounds (avoid pathological allocations)
        std.debug.assert(p32.MR <= 16);
        std.debug.assert(p64.MR <= 16);
        std.debug.assert(p32.NR <= 8);
        std.debug.assert(p64.NR <= 8);
        std.debug.assert(p32.KC <= 4096);
        std.debug.assert(p64.KC <= 4096);
        std.debug.assert(p32.MC <= 4096);
        std.debug.assert(p64.MC <= 4096);
